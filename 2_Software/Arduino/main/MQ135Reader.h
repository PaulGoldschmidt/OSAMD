...
namespace s72 {
  ...
  // Vorwärtsdeklaration, weil sonst der Compiler meckert, dass er MQ135Event nicht kennt
  class MQ135Event;

  // Klassen, die auf MQ135-Events reagieren möchten, müssen von MQ135Listener erben (und die Methoden implementieren)
  class MQ135Listener {
  public:
    MQ135Listener() {}
  ...
  // liest den MQ125-Sensor aus und interpretiert dessen Werte
  // benötigt zur Korrektur die Temperatur und lauscht daher auf DHT-Ereignisse
  ...
  public:
    ...
    static unsigned long const PREHEAT_DURATION_MS = 15 * 60 * 1000;  // Vorheizzeit des MQ135: 15 Minuten
    static unsigned long const READ_DELAY_MS = 1000;  // Zeit zwischen zwei Lesevorgängen
  ...
  // Ein Event, der die ausgelesenen Daten des MQ135-Sensors enthält
  class MQ135Event
  {
    friend class MQ135Reader; // nur der MQ135Reader darf neue Instanzen der Klasse erzeugen

  private:
    static MQ135Listener *listener; // ACHTUNG! nur genau ein Listener (spart RAM)
